...
import { AlertTriangle, Search, Eye, CheckCircle, Package } from 'lucide-react';
...
import { closeDispute, getAllDisputes, getAllOrders, resolveDispute } from '@/services/adminService';
...
const AdminDisputes = () => {
  ...
  const loadDisputes = async () => {
    try {
      ...
      const [allDisputes, allOrders] = await Promise.all([getAllDisputes(), getAllOrders()]);
      setDisputes(allDisputes);
      setOrders(allOrders);
    } catch (error) {
      console.error('[AdminDisputes] Failed to load disputes', error);
  ...
  const filteredDisputes = useMemo(() => disputes.filter(dispute => {
    const matchesSearch = 
  ...
  const handleResolve = async () => {
    ...
    try {
      await resolveDispute(selectedDispute.id, user.id, resolution, outcome);
      toast({
      ...
      console.error('[AdminDisputes] Failed to resolve dispute', error);
      toast({
  ...
  const handleClose = async (dispute: Dispute) => {
    if (window.confirm('Are you sure you want to close this dispute?')) {
      try {
        ...
        console.error('[AdminDisputes] Failed to close dispute', error);
        toast({
      ...
      <div className="space-y-6 animate-fade-up">
        ...
        {loading ? (
          <div className="farm-card text-center py-12">
            <AlertTriangle className="w-12 h-12 text-muted-foreground mx-auto mb-4 animate-pulse" />
            <p className="text-muted-foreground">Loading disputes from Supabase...</p>
          </div>
        ) : errorMessage ? (
          <div className="farm-card text-center py-12">
            <AlertTriangle className="w-12 h-12 text-destructive mx-auto mb-4" />
            <p className="text-foreground font-medium mb-2">Could not load disputes</p>
